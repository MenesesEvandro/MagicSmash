...
- **Optional doodle mode** — a finger or the mouse paints a bright, glowing line across the screen while it's down, layered on top of whichever theme is playing rather than a theme of its own, so every ordinary tap effect, sound, and animation still fires right along with it. Each stroke stays visible until the doodle canvas auto-clears after roughly 12 seconds of no drawing, or clears instantly the moment the mode is switched off — or, with the separate "keep the drawing" setting on, it doesn't auto-clear at all, staying put until an explicit Clear, a shake, or the mode being switched off wipes it. Settings also has an explicit Clear button, and a Save button that downloads the current drawing as a `.png` — nothing leaves the device.
- **Optional shake-to-clear**, Etch A Sketch style — shaking the device wipes the background, any sparkles or theme effects still mid-animation, and the doodle canvas, with a soft whoosh, distinct from ending the session itself. On iOS this needs one explicit permission grant, requested only when the setting is turned on.
...
- **Session timer** — 3, 5, 10, 15, 30, 45, or 60 minutes, or no limit, set in Settings behind the parent gate so a curious toddler can't stretch their own playtime. Ends in a friendly recap screen. Opening Settings or Stats mid-session pauses the clock, so checking on something doesn't eat into playtime.
...
- **A parent gate, on by default** — opening Settings or Stats, welcome screen or mid-session, takes a deliberate 2-second hold on the button (with a progress fill as feedback) instead of a tap, so a toddler's random smacks can't change settings, set a longer playtime for themselves, or wander out of the game. Holding Enter or Space on the focused button works too, a second finger mashing nearby can't interrupt the hold, and the language selector locks mid-session so no top-bar control is left ungated. A blocked tap briefly shows a "hold to open" hint, switching between Settings and Stats once the panel is already open needs no second hold, and the gate itself can be turned off in Settings.
