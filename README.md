...
- **Optional edge dead zone** — a toddler often grips the tablet by its edges to hold it steady, and those grip points land on the screen right along with intentional taps. Turning this setting on ignores touches within a small margin of the play area's edges, so a bracing thumb along the side doesn't register as a tap and doesn't count toward a Super Smash either — only touches nearer the middle of the screen do.
